...
class BatchConfigAction(BaseActionView):
    action_name = "configure_selected"
    description = ugettext_lazy(
        u'批量配置选择的 %(verbose_name_plural)s')

    model_perm = 'change'
    icon = 'fa fa-gear'

    def do_action(self, queryset):
        query_result = queryset.values('IP_MDCN','IP_INTERNAL','IP_MANAGE')
        dicttojson = {}
        dicttojson['IP_MDCN'] = []
        dicttojson['IP_INTERNAL'] = []
        dicttojson['IP_MANAGE'] = []
        for item in query_result:
            dicttojson['IP_MDCN'].append(item['IP_MDCN'])
            dicttojson['IP_INTERNAL'].append(item['IP_INTERNAL'])
            dicttojson['IP_MANAGE'].append(item['IP_MANAGE'])
        #return HttpResponse(json.dumps(dicttojson))
        return render_to_response('configure.html',{'query_result': query_result,'json_data':json.dumps(dicttojson)})
        #return HttpResponse(queryset.values('IP_MDCN','IP_INTERNAL','IP_MANAGE'))
        #return render_to_response('configure.html',{'query_result': query_result})
        #return HttpResponseRedirect('/app/batch_config',{'query_result': query_result})
